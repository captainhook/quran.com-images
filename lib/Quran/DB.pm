...
sub get_page_lines {
	...
	if (!defined $self->{_select_page_glyphs}) {
		$self->{_select_page_glyphs} = $self->{_dbh}->prepare_cached(
			...
			"g.glyph_code, gt.name, g.glyph_type_id FROM glyph_page_line gpl LEFT JOIN glyph g ON ".
	...
	while (my ($glyph_page_line_id, $line_number, $line_type, $glyph_position,
			$line_font, $glyph_code, $glyph_type, $glyph_type_id) = $self->{_select_page_glyphs}->fetchrow_array) {
